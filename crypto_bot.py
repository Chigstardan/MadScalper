...
def applytechnicals(df, dx):
	...
	dx['ema5'] = ta.trend.ema_indicator(dx.Close, window=5)
	dx['ema10'] = ta.trend.ema_indicator(dx.Close, window=10)
	dx['ema15'] = ta.trend.ema_indicator(dx.Close, window=15)
...
class Signals:
	def __init__(self, df, dx):
		...
		self.dx = dx	
	def decide(self):
		self.df['Buy'] = np.where((self.df.ema5[-1] > self.df.ema15[-1])
		                       & (self.df.ema10[-1] > self.df.ema15[-1])
		                       & (self.df.ema5[-1] > self.df.ema10[-1])
		                       & (self.df.ADX > 20)
		                       & (self.df.Open.iloc[-2] < self.df.Close.iloc[-2])
		                       & (self.df.Low[-2] < self.df.ema10[-2] )
		                       & (self.df.macd[-2] > 0), 1, 0)
		self.df['Sell'] = np.where((self.df.ema5[-1] < self.df.ema15[-1])
		                          & (self.df.ema10[-1] < self.df.ema15[-1])
		                          & (self.df.ema5[-1] < self.df.ema10[-1])
		                          & (self.df.ADX > 20)
	                          	& (self.df.Open.iloc[-2] > self.df.Close.iloc[-2])
	                          	& (self.df.High[-2] > self.df.ema10[-2])
	                          	& (self.df.macd[-2] < 0), 1, 0)
		self.dx['HBuy'] = np.where((self.dx.ema5 > self.dx.ema15)
		                       & (self.dx.ema10 > self.dx.ema15)
		                       & (self.dx.ema5 > self.dx.ema10)
		                       & (self.dx.rsi > 50)
		                       , 1, 0)
		self.dx['HSell'] = np.where((self.dx.ema5 < self.dx.ema10)
	                          	& (self.dx.ema5 < self.dx.ema15)
	                          	& (self.dx.ema10 < self.dx.ema15)
	                          	& (self.dx.rsi < 50), 1, 0)
